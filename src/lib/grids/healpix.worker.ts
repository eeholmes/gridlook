...
async function buildGrid(request: THealpixWorkerRequest) {
  ...
  // Same point in the pipeline as decodeVariableDataAndGetBounds: right after
  // CF decoding, so texture, histogram, bounds and hover all agree.
  applyValueTransformInPlace(data, request.valueTransform);
